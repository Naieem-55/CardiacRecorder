...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final Button showButton = findViewById(R.id.button3);



        final String phoneNumber = Login.phoneNo;  // Phone No taken from Login page

        DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReferenceFromUrl("https://cardiacrecorder-ee617-default-rtdb.firebaseio.com/").child("users");
        ...
        deleteButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                databaseReference.child("users").child(phoneNumber).addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        databaseReference.child(phoneNumber).child(String.valueOf(serialNo)).child("Date").setValue("");
                        databaseReference.child(phoneNumber).child(String.valueOf(serialNo)).child("Time").setValue("");
                        databaseReference.child(phoneNumber).child(String.valueOf(serialNo)).child("Systolic").setValue("");
                        databaseReference.child(phoneNumber).child(String.valueOf(serialNo)).child("Diastolic").setValue("");
                        databaseReference.child(phoneNumber).child(String.valueOf(serialNo)).child("HeartRate").setValue("");
                        databaseReference.child(phoneNumber).child(String.valueOf(serialNo)).child("Comment").setValue("");
                    }
